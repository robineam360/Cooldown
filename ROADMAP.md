...
- **Status:** **Shipped v1.7 (2026-09-25).** **Built 2026-09-17** · **Verified on the Fold 7, 2026-09-17** (cover and inner screens, dark and light; captures in `design/research/2026-09-17-v17-device-pass/`).
...
Weekly, `resetsAt − 24 h` — live ticking begins; (d) `fetchedAt + 24 h` — the "as of" stamp
gains its weekday (R2), for every placed widget's shown account (added at Step 4 on a Fable call,
2026-09-25: the stamp is the one time that moves *away* from now, so without it a face last fed
yesterday reads as today); null when nothing is placed. `Surfaces.arm(context)`
...
- **Status:** **Verified on the Fold 7 2026-09-26** (RUNBOOK.md Step 7 re-run over USB, release build of 20328c3; captures in `design/research/2026-09-26-v18-device-pass/` 20–52): Ring 1×1 and 2×2 seen on both screens in every placed state · **Built 2026-09-25** (RUNBOOK.md Step 4; device pass at Step 7) · was Planned · medium · **wireframe rev D approved 2026-09-25** — Q10: the spent × at 100% on
...
- **Status:** **Verified on the Fold 7 2026-09-26** (RUNBOOK.md Step 7 re-run over USB, release build of 20328c3; captures in `design/research/2026-09-26-v18-device-pass/` 20–52): 126 tiles, every face × bucket × S1–S14, render with no clipping in the release build (`33-usb-gallery-sheet0…5.jpg`) · Built 2026-09-25 (RUNBOOK.md Step 5) · **wireframe rev D approved 2026-09-25** (`design/2026-09-25-widgets-reborn.html` §9d: tiles captioned "Ring 2×2 · S3 · 14 KB") (debug-only screen) · `FacesGalleryActivity`, `android:exported="false"`, entry in Settings → Debug wired by the seat · supersedes CCBG-19 (Fixture
...
- **Status:** **Shipped v1.7 (2026-09-25).** **Verified on the Fold 7, 2026-09-17** — the block reads "This plan fits how you use it. Between 25% and 75% most weeks · capped 2 times / 5h cap hit 12 times" on the Team Premium account, in the chip-toggle pane and the inner screen's side-by-side pane. Built 2026-09-16 (`b6d592a`, `b827dc5`).
...
- **Status:** **Shipped v1.7 (2026-09-25).** **Verified on the Fold 7, 2026-09-17** — the Reorder sheet lists the three accounts with marks, plan chips and handles. Built 2026-09-16 (`b6d592a`). The index-keyed tab trap named below was real and is fixed: Main and History now store `profile.key`.
...
- **Status:** **Shipped v1.7 (2026-09-25).** **Shipped and verified on the Fold 7, 2026-09-16.** With the honest
...
- **Status:** **Shipped v1.7 (2026-09-25).** **Built and verified on the Fold 7, 2026-09-16.** `dumpsys` reports `PinnedService`
...
- **Status:** **Shipped v1.7 (2026-09-25).** Done (2026-08-19) · design record
  `design/diagnostics-shortcuts-wireframe.html`, built under the same-day
...
- **Status:** **Verified on the Fold 7 2026-09-26** (RUNBOOK.md Step 7 re-run over USB, release build of 20328c3; captures in `design/research/2026-09-26-v18-device-pass/` 20–52): the ⋮ reads Details… · Clear usage history… · Rename… · Accent colour… · Remove account; the dialog names both stores and the account, and Cancel closes it with nothing cleared; the destructive branch was not run (no account named expendable) and rests on the unit test · **Built 2026-09-25** (RUNBOOK.md Step 5; device pass at Step 7): the ⋮ item, `ClearHistoryDialog` and `UsageRepository.clearHistory` (both stores, then `Surfaces.refresh`); shown on a signed-out card too, since Clear keeps history · was Planned · **in the v1.8 arc** · placement decided 2026-09-25 (Robin): "Clear usage history…" in the Accounts-tab card ⋮ beside Details and Remove, confirmed in the `RemoveAccountDialog` shape naming both stores; calls `HistoryStore.clear` + `SessionLog.clear` + `Surfaces.refresh`. **Wireframe rev D approved 2026-09-25** (§9b): the ⋮ reads Details… · Clear usage history… · Rename… · Accent colour… · Remove account; the dialog "Clear usage history?" names "8 days of trend history" and "a year of session and weekly history" for that account only, says the account, its sign-in and settings stay, and confirms with "Clear history"
...
- **Status:** **Verified on the Fold 7 2026-09-26** (RUNBOOK.md Step 7 re-run over USB, release build of 20328c3; captures in `design/research/2026-09-26-v18-device-pass/` 20–52): Above pace, At 100% and No data drive every placed face; the ribbon shows on faces ≥160 dp tall, the dot on the shorter; the Main banner turns it off and the faces return to real data; a reboot resets it to Off (`am kill` cannot take the process while the pinned notification's foreground service runs — not a defect, and not force-stop) · **Observed 2026-08-04** · synthetic series **Built 2026-09-25** (RUNBOOK.md Step 5; device pass at Step 7): `data/SyntheticSeries` applied at the two read seams every surface draws from, `UsageCache.snapshot` and `HistoryStore.points` (the notification and the widgets read the cache directly, so the repository alone would have missed them); alerts, reset checks and the fetch read `realSnapshot`, so a synthetic 100% never fires a real alert; sign-in state stays real; the series is anchored when switched on; the History screen's closed-window bars (SessionLog) stay real; `SyntheticSeriesTest` proves no store changes · **collapsed notification, rev E (2026-09-25; Fable, checked by Astra):** the collapsed row was too short for rev D's band (the 32 sp figure clipped), so collapsed views drop it — the single row ends its title line with a 7 dp #B388FF dot, the Duet puts a 6 dp one on its First half beside the condition dot (`Duet.labelClampDp` counts 11 dp per dot), both read "Synthetic data" to TalkBack; the builder's content text is prefixed "Synthetic data · " for surfaces that never draw the custom views; expanded views keep the band, and the expanded Duet draws no condition strip under synthetic (band + strip overrun its cap). Emulator (API 36): dot and TalkBack label seen on the single row; the Duet is unit-tested only. Seen there with synthetic Off too, so not rev E's: the collapsed row's third (reset/strip) line clips on the Pixel skin — check the Fold 7 at Step 7 · synthetic-series override was planned **in the v1.8 arc** (RUNBOOK.md Step 5; rule R8 of the v1.8 section, 2026-09-25): a Debug-section chip row "Synthetic series: Off / Above pace / At 100% / No data" in the **release** build behind the 7-tap unlock; a process-wide in-memory holder `data/SyntheticSeries`, applied where `UsageRepository` hands out a snapshot so chart, bars, rings, the notification and the widgets render one series; never written to a store (unit-tested); marked wherever it shows — a SYNTHETIC DATA banner on Main that is also the off switch, a strip on the notification, a marker on every widget face (**rev D, 2026-09-25**: the violet "SYNTHETIC" ribbon on faces 160 dp tall or more, a 7 dp violet dot on the shorter ones; the notification carries a full-width "SYNTHETIC DATA" band above its row; the Main banner reads "SYNTHETIC DATA · tap to turn off"); resets to Off with the process. The harness half moves to CCRM-84 (Faces Gallery)
...
- **Status:** **Verified on the Fold 7 2026-09-26** (RUNBOOK.md Step 7 re-run over USB, release build of 20328c3; captures in `design/research/2026-09-26-v18-device-pass/` 20–52): the preview renders ring, bars with pace and resets, the full 5h trend and the wordmark (`49-usb-inner-share-preview.png`); Share opens the system chooser; not sent · **Built 2026-09-25** (RUNBOOK.md Step 6) — not yet seen on a device; the Step 7 pass shares one from the Fold 7. Renders: [design/2026-09-25-share-card-renders.html](design/2026-09-25-share-card-renders.html). Calls made at the build (Fable, checked by Astra): Share works in synthetic mode and the card carries the widgets' violet "● SYNTHETIC" ribbon, keyed to the drawn snapshot's own provenance; the ⋮ shows whenever the open account has a reading ("Main screen layout" still needs ≥2 cards); the PNG is square and full-bleed (the preview rounds it). The trend is the full chart (guides, pace diagonal, axis, dots, now line, projection) at 120 dp rather than rev D's sparkline — Fable and Astra split, **Robin chose the full chart**. Planned · medium · **in the v1.8 arc** (Robin, 2026-09-25: wireframe now, build last, ships in v1.8 only if ready — the release does not wait). **Wireframe rev D approved 2026-09-25 — build** (RUNBOOK.md Step 6). **Scope: only the account open when Share is tapped** (Robin, against the all-accounts recommendation): an 88 dp ring with the figure, the name and "as of 6:29 PM, Thu Sep 25", the 5h and Weekly bars with pace readouts and absolute resets, that account's 5h trend, a small COOLDOWN wordmark. Gate lifted by CCRM-83 (Ring Renderer); `share/ShareCard.kt` at 4× (1440 px wide) from `RingRenderer`, `BarRenderer` and a `ChartBitmap` read from `530781f`; render-then-preview, then `ACTION_SEND` with `FLAG_GRANT_READ_URI_PERMISSION` on a `FileProvider` cache-path URI (`cacheDir/share/`, earlier files deleted before each render, the folder emptied on app start; nothing written outside app cache); entry in the Main ⋮ menu ("Share snapshot"). Privacy rules below unchanged
...
- **Status:** **Shipped v1.7 (2026-09-25).** **Verified on the Fold 7, 2026-09-17** (built under CCRM-72 (Main Screen Redesign)). **Building under CCRM-72 (Main Screen Redesign), 2026-09-17** — the *Reset layout* button on the layout sheet, confirmed in the `RemoveAccountDialog` shape. Wireframe rev E approved 2026-09-17.
