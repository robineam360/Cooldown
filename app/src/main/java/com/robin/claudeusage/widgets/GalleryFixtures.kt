 ...
 *
 * Moved into main source for CCRM-84 (Faces Gallery), whose debug screen inflates every
 * state on the phone through the same builders the tests use. `WidgetFixtures` (test)
 * delegates here so a state is built exactly one way; see that file for the delegation.
 */
