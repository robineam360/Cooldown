...
class CompactLineCopyTest {
    ...
    fun `above pace emphasises the pace clause alone`() {
        // Symmetric with the below-pace case, and short: no "even" on this surface
        // (decided on review, 2026-09-17).
        val line = compactLine("in 3d 2h", 12.0)
        assertEquals("Resets in 3d 2h · 12% above pace", line.text)
        assertEquals("12% above pace", line.boldClause)
    }

    @Test
    fun `inside the dead zone reads on pace`() {
        val line = compactLine("in 10h 1m", 2.0)
        assertEquals("Resets in 10h 1m · on pace", line.text)
        assertNull(line.boldClause)
    }

    @Test
    fun `the dead zone boundary is not above pace`() {
        assertEquals("on pace", compactPaceClause(PACE_DEAD_ZONE))
        assertEquals("on pace", compactPaceClause(-PACE_DEAD_ZONE))
    }

    @Test
    fun `an unstarted window keeps the idle copy`() {
        // CCBG-30 (Phantom Window): the same words ResetRow uses when there is no reset time yet.
    ...
    fun `the pace clause rounds rather than truncates`() {
        ...
        assertEquals("13% above pace", compactPaceClause(12.5))
    }
}
