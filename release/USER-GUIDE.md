...
> Version **1.8** · released 26 September 2026 · sideloaded personal app, not on any store
...
> orientation), the redesigned Accounts tab, and Plan Fit on History — and the v1.8 surfaces —
> home-screen widgets and the account email line — are documented in the PDF guide**, which is
> the current one: `release/docs/Cooldown-User-Guide-v1.8.pdf`.
> The Quick Settings tile and the threshold alerts described in older screenshots were
> **removed in v1.6** and stay removed. **Home-screen widgets are back as of v1.8**, as a new
> suite — anything shown removed in older screenshots doesn't return on its own.
...
> icon — and the v1.8 surfaces — home-screen widgets and the account email line — see the PDF
> guide, which carries current screenshots.
...
**Updating from v0.13 onward:** just install the new APK over it — token, settings and history all survive (releases are signed with a permanent key). **Updating to v1.6:** any placed home-screen widgets and the Quick Settings tile disappeared — they were removed; the always-on notification replaced them. **Home-screen widgets are back as of v1.8**, as a new suite — add them fresh from the widget picker; nothing from before v1.6 returns on its own, and the Quick Settings tile stays removed.
...
Home-screen widgets are back as of v1.8, as a new suite — a separate thing from the Quick Settings tile, which was removed in v1.6 and stays removed.

**Adding one:** long-press the home screen → Widgets → **Cooldown**, drag one onto a page at any size from 1×1 to a full row, and drop it. Its settings sheet opens right away — pick the **account**, the **window** (5h or Weekly) and the **background** (Solid, Gradient or Transparent). Change any of them later from the widget's own long-press **Settings**.

**The four faces** (CCRM-78 (Widgets Reborn)):

- **Ring** — one account as a gauge, filling with its window (CCRM-79 (Ring Face))
- **Number** — the big percentage, like the pinned notification (CCRM-80 (Number Face))
- **Countdown** — when the window comes back, ticking live (CCRM-81 (Countdown Face))
- **All accounts** — every account as its own ring, side by side — never a sum (CCRM-82 (Accounts Strip))

Every widget **fills whatever size you give it**: bigger Rings add the reset time and a second, smaller ring for the other window; the Number's 2×2 and 4×2 sizes carry 5h|Weekly chips and an account switcher; the Countdown's count grows with the widget (CCBG-44 (Widget Fill)). The same ring the status bar draws is now painted by one renderer everywhere (CCRM-83 (Ring Renderer)).

Widgets placed before v1.6 don't come back on their own — add the new ones fresh from the picker. If you use widgets without the always-on notification, add Cooldown to One UI's **Never sleeping apps** (Settings → Battery → Background usage limits) so they keep refreshing.
...
|---|---|
| **Accounts** | One compact card per account: mark · label · a status dot (green = active, red = needs re-auth) · plan chip · ⋮. A status line gives the essentials — *"Checked just now · renews in 1h 9m"* — and a second line appears only when something needs attention (a Free-plan notice, a stopped renewal, a rate limit, or a new amber *"Sign-in expires in 2d 14h"* inside the last 3 days). Each card also shows the **email** it signed in with, under its name (CCBG-34 (Account Display Name)). Three action cells: **Re-sign in · Clear · Dashboard** (ChatGPT: **Sign in with a code · Clear · Settings**). The card's ⋮ menu opens **Details** (last checked, last auto-renewed, expiry estimate, added, token tail) first, then Rename, Accent colour, **Clear usage history…** (CCRM-14 (Clear History)), Remove account. **"+ Add account"** and a **Reorder** button (drag accounts into the order you want) sit at the bottom. |
...
| My Quick Settings tile is gone | Removed for good in v1.6 — nothing brings it back. Home-screen widgets returned in v1.8 as a new suite (§5); add them fresh from the widget picker. |
| A widget stopped refreshing | Add Cooldown to One UI's **Never sleeping apps** (Settings → Battery → Background usage limits), especially if you don't also run the always-on notification. |
...
- **1.8** — **Home-screen widgets are back, and accounts show their email.** Four widgets in the picker under **Cooldown** — **Ring** (one account as a gauge), **Number** (the big percentage, like the pinned notification), **Countdown** (when the window comes back, ticking live) and **All accounts** (every account as its own ring, side by side — never a sum) (CCRM-78 (Widgets Reborn), CCRM-79 (Ring Face), CCRM-80 (Number Face), CCRM-81 (Countdown Face), CCRM-82 (Accounts Strip)). Adding one opens its settings — the account, the window (5h or Weekly) and the background (Solid, Gradient or Transparent) — changeable later from the widget's own long-press **Settings**; every widget **fills whatever size you place it at**, from 1×1 to a full row, painted by the same ring renderer as the status bar (CCRM-83 (Ring Renderer), CCBG-44 (Widget Fill)); widgets placed before v1.6 do not return on their own. Each account now shows the **email** it signed in with, under its name in Settings → Accounts and first in ⋮ → Details, with Rename suggesting a name from it (CCBG-34 (Account Display Name)); **Clear usage history…** in an account's ⋮ starts its trend over without removing the account (CCRM-14 (Clear History)). **Share snapshot**, in the main screen's ⋮, turns the ring, the bars with pace and resets, and the 5h trend into one square image (CCRM-24 (Share Card)). **Fixed:** the two-account notification keeps both names whole, with their dots, in Used and Left (CCBG-24 (Duet Label Clamp), CCBG-37 (Duet Dot Squeeze)); turning on the always-on notification with one account no longer crashes the app (CCBG-31 (Alert Crash)); the sign-in card's buttons wrap cleanly on narrow phones instead of squeezing "Cancel" (CCBG-32 (Accounts Button Wrap)); ChatGPT sign-in now says which security setting it needs, with a link to it (CCBG-33 (Device-Code Prerequisite)); each account card's ⋮ sits in line with its refresh button (CCBG-42 (Kebab Drift)).
...
*Verified on a real device; updated for v1.8 on 26 Sep 2026.*
